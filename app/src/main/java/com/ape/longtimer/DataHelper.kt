...
class DataHelper(context: Context) {
    private var sharedPrefs: SharedPreferences = context.getSharedPreferences(PREFERENCES, Context.MODE_PRIVATE)
    ...
    init {
        timerCounting = sharedPrefs.getBoolean(COUNTING_KEY, false)

        val startString = sharedPrefs.getString(START_TIME_KEY, null)
        if (startString != null) startTime = dateFormat.parse(startString)

        val stopString = sharedPrefs.getString(STOP_TIME_KEY, null)
